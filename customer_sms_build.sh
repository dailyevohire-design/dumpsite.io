#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# FillDirtNearMe Customer SMS System — Complete Build
# Sarah persona, Sonnet 4.6, templates control flow
# ═══════════════════════════════════════════════════════════════
set -e
cd ~/dumpsite-io || { echo "ERROR: ~/dumpsite-io not found"; exit 1; }

echo "═══════════════════════════════════════════════════════"
echo "  FillDirtNearMe Customer SMS — Full Build"
echo "═══════════════════════════════════════════════════════"
echo ""

# ═══════════════════════════════════════════════════════════════
# STEP 1: SQL MIGRATION — Run this in Supabase SQL Editor
# ═══════════════════════════════════════════════════════════════
cat > /tmp/customer_sms_migration.sql << 'SQLEOF'
-- ═══════════════════════════════════════════════════════
-- FillDirtNearMe Customer SMS — Database Migration
...
CREATE POLICY "Service role full access" ON customer_conversations FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Service role full access" ON customer_sms_logs FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Service role full access" ON customer_processed_messages FOR ALL USING (true) WITH CHECK (true);
...
echo "╚══════════════════════════════════════════════════════╝"
echo ""
echo "  File saved to: /tmp/customer_sms_migration.sql"
echo "  Copy-paste into: Supabase → SQL Editor → Run"
echo ""
echo "  Press ENTER after you've run the SQL..."
read -r

# ═══════════════════════════════════════════════════════════════
# STEP 2: CREATE CUSTOMER BRAIN SERVICE
# ═══════════════════════════════════════════════════════════════
echo "Creating customer-brain.service.ts..."

cat > lib/services/customer-brain.service.ts << 'CUSTBRAIN'
...
echo "✓ customer-brain.service.ts created"

# ═══════════════════════════════════════════════════════════════
# STEP 3: CREATE WEBHOOK ROUTE
# ═══════════════════════════════════════════════════════════════
echo "Creating customer webhook..."
mkdir -p app/api/sms/customer-webhook

cat > app/api/sms/customer-webhook/route.ts << 'WEBHOOK'
...
echo "✓ customer-webhook route created"

# ═══════════════════════════════════════════════════════════════
# STEP 4: CREATE FOLLOW-UP CRON
# ═══════════════════════════════════════════════════════════════
echo "Creating follow-up cron..."
mkdir -p app/api/cron/customer-followup

cat > app/api/cron/customer-followup/route.ts << 'FOLLOWUP'
...
echo "✓ customer-followup cron created"

# ═══════════════════════════════════════════════════════════════
# STEP 5: UPDATE VERCEL.JSON WITH CRON
# ═══════════════════════════════════════════════════════════════
...
if "/api/cron/customer-followup" not in paths:
    crons.append({"path": "/api/cron/customer-followup", "schedule": "0 */4 * * *"})
    vj["crons"] = crons
    with open("vercel.json", "w") as f:
        json.dump(vj, f, indent=2)
    print("  Added customer-followup cron (every 4 hours)")
else:
    print("  Customer followup cron already in vercel.json")
VERCELJSON
...
if [ $BUILD_EXIT -ne 0 ]; then
  ...
  echo ""
  echo "BUILD FAILED — paste the errors above"
  exit 1
fi
...
git add \
  ...
  app/api/sms/customer-webhook/route.ts \
  app/api/cron/customer-followup/route.ts \
  vercel.json
git commit -m "feat: FillDirtNearMe customer SMS — Sarah persona, pricing engine, dispatch integration, follow-up cron"
git push origin main

echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✓ CUSTOMER SMS SYSTEM DEPLOYED"
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "  REMAINING SETUP:"
echo ""
echo "  1. Run SQL migration in Supabase (file at /tmp/customer_sms_migration.sql)"
echo ""
echo "  2. Buy a new Twilio number and add to Vercel env vars:"
echo "     CUSTOMER_TWILIO_NUMBER = +1XXXXXXXXXX"
echo ""
echo "  3. Set Twilio webhook on the new number:"
echo "     https://dumpsite.io/api/sms/customer-webhook"
echo "     Method: HTTP POST"
echo ""
echo "  4. Make sure these env vars exist in Vercel:"
echo "     GOOGLE_MAPS_API_KEY (for geocoding delivery addresses)"
...
echo "  CUSTOMER FLOW:"
echo "    Customer: hi"
echo "    Sarah: Hey this is Sarah with Fill Dirt Near Me, whats your name"
echo "    Customer: John"
echo "    Sarah: John nice to meet you, whats the delivery address"
echo "    Customer: 1234 Oak St Dallas TX"
echo "    Sarah: Got it. What are you going to be using the material for"
echo "    Customer: Filling my pool"
echo "    Sarah: John for that structural fill compacts well. How many cubic yards"
echo "    Customer: I don't know"
echo "    Sarah: No problem, what are the dimensions in feet"
echo "    Customer: 20 x 40 x 6"
echo "    Sarah: That comes out to about 178 cubic yards. Dump trucks and 18 wheelers or just dump trucks"
echo "    Customer: just dump trucks"
echo "    Sarah: When do you need delivery"
echo "    Customer: next week"
echo "    Sarah: John 178 yards of structural fill delivered to Dallas comes to \$5,340. Want me to schedule"
echo "    Customer: yes"
echo "    Sarah: Whats your email for the receipt"
echo "    Customer: john@email.com"
echo "    Sarah: We accept zelle or venmo only, which works for you"
echo "    Customer: zelle"
echo "    Sarah: Send \$5,340 via Zelle to support@filldirtnearme.net. Text me once sent"
echo "    Customer: sent"
echo "    Sarah: Payment received John, delivery confirmed. You'll get a text when driver is on the way"
echo "    → dispatch_order created → drivers auto-matched"
echo ""
echo "  ARCHITECTURE:"
echo "    Templates: 100% of flow (name, address, yards, access, date, quote, payment)"
echo "    Sonnet: material recommendation (unknown purposes), off-topic, follow-up return"
echo "    Pricing: zone-based from your Excel (A=\$22, B=\$25, C=\$30 + material surcharge)"
echo "    Formula: L × W × D ÷ 27 for dimension calculation"
echo "    Safety: global try/catch, validator, dedup, never goes silent"
echo "    Follow-up: 24hr → 48hr → 72hr then closes. 3 attempts max."
echo ""
