...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await fetch('/api/dashboard/live', { cache: 'no-store' })
  ...
  useEffect(() => {
    if (!live) { intervalRef.current && clearInterval(intervalRef.current); return }
    intervalRef.current = setInterval(() => { load(); setTick(t => t + 1) }, 6000)
  ...
  const convs = data?.activeConversations || []
  const msgs = data?.liveMessages || []
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{
              ...
              background: live ? '#22c55e' : '#4b5563',
              boxShadow: live ? '0 0 10px #22c55e88' : 'none',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <button
            onClick={() => setLive(l => !l)}
            style={{
              background: live ? '#052e16' : '#1c1c1c',
              border: `1px solid ${live ? '#166534' : '#374151'}`,
          ...
          <button onClick={load} style={{
            background: '#1c1c1c', border: '1px solid #374151', color: '#9ca3af',
      ...
      <div style={{
        ...
        {[
          ...
          <div key={i} style={{
            padding: '12px 16px',
            borderRight: i < 6 ? '1px solid #1e2530' : 'none',
            background: '#0d1117',
          }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 280px', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ borderRight: '1px solid #1e2530', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '10px 16px', borderBottom: '1px solid #1e2530', background: '#0d1117' }}>
            <span style={{ color: '#6b7280', fontSize: 10, letterSpacing: 1.5 }}>
              ACTIVE DRIVERS ({convs.length})
            </span>
          </div>
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {convs.length === 0 && (
              <div style={{ color: '#374151', padding: 20, textAlign: 'center', fontSize: 12 }}>
            ...
            {convs.map((c) => {
              ...
              const lastMsg = byPhone[c.phone]?.slice(-1)[0]
              return (
                <div
                  key={c.phone}
                  onClick={() => setSelected(isSelected ? null : c.phone)}
                  style={{
                    padding: '12px 16px',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{
                      ...
                      background: st.color + '22', color: st.dot,
                      border: `1px solid ${st.color}44`,
                      letterSpacing: 0.5,
                    }}>{st.label}</span>
                  </div>
                  <div style={{ display: 'flex', gap: 8, marginTop: 5 }}>
                    {c.extracted_city && <span style={{ color: '#6b7280', fontSize: 10 }}>pin:{c.extracted_city}</span>}
                    {c.extracted_truck_type && <span style={{ color: '#6b7280', fontSize: 10 }}>truck:{truck(c.extracted_truck_type)}</span>}
                    {c.extracted_yards && <span style={{ color: '#6b7280', fontSize: 10 }}>yds:{c.extracted_yards}</span>}
                  </div>
                  {lastMsg && (
                    <div style={{
                      marginTop: 5, color: '#4b5563', fontSize: 10,
        ...
        {/* CENTER: LIVE SMS FEED */}
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{
            padding: '10px 16px', borderBottom: '1px solid #1e2530',
            background: '#0d1117', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          }}>
            <span style={{ color: '#6b7280', fontSize: 10, letterSpacing: 1.5 }}>
              {selected ? `CONVERSATION +1${selected}` : 'LIVE SMS FEED'}
            </span>
            {selected && (
              <button onClick={() => setSelected(null)} style={{
                background: 'transparent', border: '1px solid #374151', color: '#6b7280',
                padding: '2px 8px', borderRadius: 3, cursor: 'pointer', fontSize: 10, fontFamily: 'inherit',
              }}>x ALL</button>
            )}
          </div>
          <div ref={feedRef} style={{ overflowY: 'auto', flex: 1, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {selectedMsgs.slice(-60).map((m, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  fontSize: 9, color: '#374151', marginBottom: 2,
                  display: 'flex', gap: 6,
                }}>
                ...
                <div style={{
                  maxWidth: '72%',
                  background: m.direction === 'outbound' ? '#0f2040' : '#111827',
                  border: `1px solid ${m.direction === 'outbound' ? '#1e3a5f' : '#1f2937'}`,
                  borderRadius: m.direction === 'outbound' ? '10px 2px 10px 10px' : '2px 10px 10px 10px',
                  padding: '7px 12px',
                  color: m.direction === 'outbound' ? '#93c5fd' : '#d1d5db',
                  fontSize: 12,
                  lineHeight: 1.5,
            ...
            {selectedMsgs.length === 0 && (
              <div style={{ color: '#374151', textAlign: 'center', padding: 40, fontSize: 12 }}>
                {selected ? 'No messages for this driver' : 'No messages yet'}
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ borderLeft: '1px solid #1e2530', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ borderBottom: '1px solid #1e2530' }}>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid #111827', background: '#0d1117' }}>
              <span style={{ color: '#6b7280', fontSize: 10, letterSpacing: 1.5 }}>PIPELINE</span>
            </div>
            <div style={{ padding: '8px 0' }}>
              {Object.entries(STATE).map(([state, cfg]) => {
                const count = convs.filter(c => c.state === state).length
                ...
                return (
                  <div key={state} style={{
                    ...
                    <span style={{
                      color: cfg.dot, fontWeight: 700, fontSize: 13,
                      background: cfg.color + '22', padding: '1px 8px', borderRadius: 3,
                    }}>{count}</span>
                  </div>
                )
              })}
          ...
          <div style={{ borderBottom: '1px solid #1e2530' }}>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid #111827', background: '#0d1117' }}>
              <span style={{ color: '#6b7280', fontSize: 10, letterSpacing: 1.5 }}>ORDERS (24H)</span>
            </div>
            <div style={{ padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {Object.entries(stats?.statusCounts || {}).map(([status, count]) => (
                <div key={status} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div style={{
                    height: 3, width: Math.min((count as number) * 10, 100),
                    background: status === 'completed' ? '#22c55e' : status === 'active' ? '#f59e0b' : '#4f46e5',
                    borderRadius: 2,
                  }}/>
          ...
          <div style={{ padding: '10px 16px', borderBottom: '1px solid #111827', background: '#0d1117' }}>
            <span style={{ color: '#6b7280', fontSize: 10, letterSpacing: 1.5 }}>PAYMENTS (24H)</span>
          </div>
          <div style={{ overflowY: 'auto', flex: 1 }}>
            <div style={{ padding: '8px 16px', display: 'flex', gap: 12, borderBottom: '1px solid #111827' }}>
              <div>
                <div style={{ color: '#374151', fontSize: 9 }}>PENDING</div>
                <div style={{ color: '#f97316', fontSize: 18, fontWeight: 700 }}>${(pay?.pending || 0).toFixed(0)}</div>
              </div>
              <div style={{ width: 1, background: '#1f2937' }}/>
              <div>
                <div style={{ color: '#374151', fontSize: 9 }}>PAID</div>
                <div style={{ color: '#22c55e', fontSize: 18, fontWeight: 700 }}>${(pay?.paid || 0).toFixed(0)}</div>
              </div>
            </div>
            {(pay?.records || []).slice(0, 15).map((p, i) => (
              <div key={i} style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '6px 16px', borderBottom: '1px solid #0d1117',
              }}>
                <div>
                  <div style={{ color: '#6b7280', fontSize: 10 }}>{fmt(p.driver_phone)}</div>
                  <div style={{ color: '#374151', fontSize: 9 }}>{ago(p.created_at)}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ color: p.status === 'paid' ? '#22c55e' : '#f97316', fontSize: 12, fontWeight: 700 }}>
                    ${(p.amount_cents / 100).toFixed(0)}
                  </div>
                  <div style={{
                    fontSize: 9, color: p.status === 'paid' ? '#166534' : '#92400e',
                    background: p.status === 'paid' ? '#052e16' : '#1c0a00',
