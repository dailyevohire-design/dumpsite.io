...
# ─── 2. Admin dispatch DELETE route ──────────────────────────────────────────
os.makedirs(f'{BASE}/app/api/admin/dispatch', exist_ok=True)
# The route.ts already exists for GET/POST, we need to add DELETE to it

with open(f'{BASE}/app/api/admin/dispatch/route.ts', 'r') as f:
    existing = f.read()
...
if 'DELETE' not in existing:
    with open(f'{BASE}/app/api/admin/dispatch/route.ts', 'w') as f:
        f.write(existing.rstrip() + '\n' + delete_route)
    print('✅ admin/dispatch/route.ts — DELETE added')
else:
    print('ℹ️  DELETE already exists in dispatch route')

# ─── 3. Submit-load route — add server-side validation ────────────────────────
submit_route = """import { NextRequest, NextResponse } from 'next/server'
...
os.makedirs(f'{BASE}/app/api/driver/submit-load', exist_ok=True)
with open(f'{BASE}/app/api/driver/submit-load/route.ts', 'w') as f:
    f.write(submit_route)
print('✅ api/driver/submit-load/route.ts written')

print('\n✅ ALL STABILITY FIXES WRITTEN')
