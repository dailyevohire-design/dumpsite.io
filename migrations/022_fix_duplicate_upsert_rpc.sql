-- 022_fix_duplicate_upsert_rpc.sql
-- Fix: Two overloaded versions of upsert_customer_conversation exist in the database.
-- PostgREST cannot disambiguate them, so ALL saveConv() calls silently fail.
-- This caused customer conversations to never persist — every SMS was treated as NEW.
--
-- Solution: Drop BOTH overloads, then recreate the single correct version.

-- Drop the stale overload that has p_pricing_type (was created manually or from a deleted migration)
DROP FUNCTION IF EXISTS upsert_customer_conversation(
  ...
  text,  -- p_pricing_type (the extra param)
  text, text, text, uuid, timestamptz, integer
);

-- Drop the current overload too (so CREATE OR REPLACE works cleanly)
DROP FUNCTION IF EXISTS upsert_customer_conversation(
  text, text, text, text, text, text,
  double precision, double precision,
  text, text, integer, text, text, text, text,
